...
lookback=-40 #must be a negative value
...
class CNN_LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_stacked_layers,output_size):
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=32, kernel_size=4, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Dropout(0.5),
            nn.Conv1d(in_channels=32, out_channels=input_size, kernel_size=4, stride=1, padding=2),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2),
            nn.Dropout(0.2)
        )
...
for i,name in enumerate(data_attributes[:]):
    for j in range(lookforward):
        full_shifted_df[f'{name}(t+{j})']=full_shifted_df[f'{name}(t+{j})'].shift(j)
