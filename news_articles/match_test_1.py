...
def append_to_csv(ticker, matched_names, article):
    output_file = f'yahoo_matched_articles/{ticker}_match.csv'
...
def process_chunk(chunk, processed_data):
    for index, row in tqdm(chunk.iterrows(), total=chunk.shape[0], desc="Processing"):
        ...
        for ticker, value in processed_data.items():
            ...
            #     if attribute == 'products':  # Special handling for products
            #         for name in names:
            #             if name.isupper():
            #                 if name in article_text:
            #                     matches.append(name)

            #                 # Check if product name is not just lowercase (which might be a category)
...
if __name__ == '__main__':
    ...
    # Read the news CSV file
    news_df = pd.read_csv('yahoo_articles_all_cleaned.csv')
